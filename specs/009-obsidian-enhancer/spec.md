...
- 自动移动匹配时，tag 段与目录段的分隔形式差异（空格、连字符、下划线）不应导致误判未命中。
- 自动移动匹配时，若排除根目录规则导致未命中，应提供可回退策略以避免“配置过严导致永不匹配”。
...
- **FR-009**: 系统必须在自动移动匹配中对 tag 段与目录段进行一致化归一（至少覆盖大小写、空格、连字符、下划线及目录约定前缀），避免语义相同但格式不同导致的匹配失败。
- **FR-010**: 系统必须在自动移动匹配中支持“按排除规则优先匹配，失败后回退到全量目录再次匹配”的策略，并保持结果可解释。
