...
3. 同名文件在不同子目录中不会互相覆盖。  
4. 任一文件失败时，其余文件继续执行。  
5. 并发执行时终端可同时看到多个活跃文件子进度（数量与并发池一致），且全局进度条与当前文件进度条颜色不同。  
6. PDF 文件进度遵循：先 `pdfinfo` 取页数，再一次性 `pdftoppm` 转换，完成后进度直接到 100%。  
7. 当前文件进度条后方展示相对路径（不展示绝对路径）。  
8. 运行期间日志不会打断进度条显示（无错位、无被普通日志插入）。  
9. 终端结束摘要为英文、结构化排版、可含 emoji，并包含成功数、失败原因、输出目录、总耗时且状态具备多色区分。
