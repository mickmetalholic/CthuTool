...
- 最终总结固定使用英文输出，采用结构化终端布局（标题、统计区、失败列表区），允许使用 emoji 强化信息分组。
- 总结阶段继续复用同一输出通道，通过统一颜色语义映射区分 `success` / `warn` / `error` / `info`。
- 全局进度条与当前文件进度条使用不同颜色，避免与状态日志颜色混淆。

### Rationale

- 英文总结与结构化排版可提高跨团队可读性，也便于后续 CI 日志检索。
- 颜色语义统一可以降低用户误判，提升长任务下的信息扫描效率。
- 进度条颜色独立于日志颜色，可确保用户在并发场景快速识别“总体 vs 当前”。
