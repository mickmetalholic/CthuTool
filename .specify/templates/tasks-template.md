...
**Tests**: Per CthuTool Constitution, TDD is mandatory: failing tests before implementation for new logic.
Include test tasks for every user story and for pure/core business functions; use Jest (Next.js/NestJS) or
`bun test` (Bun) per the package being changed.
...
- [ ] T008 Configure `neverthrow`-based Result patterns and structured logging at infrastructure boundaries
...
- [ ] T016 [US1] Add valibot schemas and neverthrow Result composition at boundaries
...
- [ ] TXXX [P] Additional unit tests in tests/unit/ (required for new pure/core logic per constitution)
...
```bash
# Launch all tests for User Story 1 together (TDD — before implementation):
