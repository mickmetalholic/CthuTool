...
- **WHEN** the user runs `cthutool-cli scripts convert-to-cbz`
- **THEN** the `scripts` command selects the bundled script with id `convert-to-cbz`

#### Scenario: Script flag
- **WHEN** the user runs `cthutool-cli scripts --script convert-to-cbz`
- **THEN** the `scripts` command selects the bundled script with id `convert-to-cbz`
...
- **WHEN** the user runs `cthutool-cli scripts convert-to-cbz --input ./samples --format jpg`
...
- **WHEN** `cthutool-cli scripts convert-to-cbz --input ./samples --json` completes successfully
