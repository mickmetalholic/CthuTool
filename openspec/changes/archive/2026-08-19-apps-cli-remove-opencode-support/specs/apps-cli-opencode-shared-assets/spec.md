...
**Reason**: CthuTool will no longer synchronize repository plugin assets into OpenCode.
**Migration**: Continue using `chc codex install` and `chc codex skills` for Codex-managed plugin assets. Existing OpenCode configuration is not modified.
...
**Reason**: The `chc opencode skills` command is being removed as part of retiring the OpenCode adapter.
**Migration**: There is no CthuTool replacement for managing OpenCode skill paths. Use Codex's native plugin and skill commands where Codex is the target agent.
...
**Reason**: CthuTool will no longer translate plugin MCP declarations into OpenCode configuration.
**Migration**: There is no CthuTool replacement for OpenCode MCP synchronization. Existing OpenCode MCP entries remain under the user's control.
...
**Reason**: CthuTool no longer reads or writes OpenCode configuration files.
**Migration**: Existing OpenCode JSON/JSONC files are left in place and can continue to be managed by OpenCode or other tools.
...
**Reason**: The public OpenCode command boundary and its output contract are being retired.
**Migration**: `chc codex install` remains the supported plugin installation command, and unsupported `chc opencode` invocations should be removed from user scripts.
