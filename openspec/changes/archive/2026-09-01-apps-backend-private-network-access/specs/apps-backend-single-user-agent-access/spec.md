...
**Reason**: The fixed private-network boundary removes both the operator
gateway identity header and the static Agent WebSocket secret from the
single-user deployment.
**Migration**: Use the private network for Backend-side access control and
Cloudflare Access/Tunnel for external operator authentication. Environment id
remains routing metadata, not a credential.

### Requirement: Minimal static-secret lifecycle
**Reason**: The Backend no longer supports per-environment Agent secrets,
manual secret replacement, or secret-specific access configuration.
**Migration**: Remove static-secret configuration and CLI workflows. Existing
local secret files are not read or deleted automatically; they may be removed
manually after the new client release is verified.
