...
The backend SHALL keep existing browser-facing site config behavior compatible while moving provider ownership into `SitesConfigModule`.

#### Scenario: Browser sites API is unchanged
- **WHEN** a client calls `GET /api/browser/sites`
...
#### Scenario: Browser automation imports sites config
- **WHEN** browser automation services need site configuration for content resolution or API responses
- **THEN** they consume `SitesConfigService` from `SitesConfigModule` rather than registering their own site config provider
