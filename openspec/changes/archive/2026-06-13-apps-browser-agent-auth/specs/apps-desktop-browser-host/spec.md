...
CthuDesktop SHALL publish non-sensitive local browser state snapshots to the backend after connection and after local profile or pending-auth state changes.
...
- **THEN** it publishes local profile summaries and pending auth tasks to backend APIs without including cookies, storage-state contents, localStorage values, or profile paths
...
- **THEN** CthuDesktop publishes the updated non-sensitive state projection to the backend
