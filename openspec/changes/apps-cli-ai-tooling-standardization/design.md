...
The repository also has two distinct skill systems that must remain separate:

- OpenSpec owns its workflow skills and tool-specific command/skill generation.
- `npx skills` owns third-party and project-authored reusable skills.
...
The existing project policy says generated agent adapters are regenerated per tool/platform rather than hand-authored. The design therefore treats `openspec/` and the canonical project skill sources as the durable inputs, and generated agent directories as reproducible outputs.
...
- Provide a small canonical set of tool-neutral project skills for repository orientation, validation, and diff review.
...
Use the current OpenSpec CLI generation model and the core workflow set. Configure `codex`, `cursor`, `opencode`, and `agents`, then regenerate with `openspec update` after CLI or profile changes. OpenSpec-managed names are limited to `openspec-*`; custom project skills and unrelated agent files are preserved.
...
The repository documents `nextlevelbuilder/ui-ux-pro-max-skill` as the verified upstream source and selects only `ui-ux-pro-max` when a developer explicitly wants it. The default project baseline contains no UI/UX skill. Project-authored reusable skills live in a canonical `skills/` source tree and are linked or copied to selected agents through a reproducible setup command.
...
### Baseline skills are small and tool-neutral

Start with `repo-orientation`, `project-verify`, and `review-diff`. Their canonical instructions must avoid tool-specific primitives such as `AskUserQuestion`, `TodoWrite`, or a particular slash-command spelling. Git commit and pull-request actions remain explicit-only capabilities and are documented separately from read-only/review skills.

Alternative considered: install a large marketplace bundle for every tool. Rejected because it increases context load and amplifies capability drift before the basic lifecycle is reliable.

## Risks / Trade-offs

- [Generated outputs are absent after a fresh clone] → Add a documented, idempotent setup command that installs OpenSpec and links the selected skills for all four tools; verify the generated paths in CI or a repository doctor command.
- [OpenSpec upgrade removes or relocates legacy files] → Run `openspec update --force` only after reviewing the generated diff; limit cleanup to OpenSpec-managed names and preserve unrelated project skills.
- [Reasonix and other agents interpret skill frontmatter differently] → Keep the baseline frontmatter minimal (`name` and `description`), run a per-tool discovery smoke test, and avoid optional agent-specific fields in shared skills.
- [Symlinks are inconvenient on some platforms] → Make the setup command support the Skills CLI copy mode while keeping one canonical source; document the selected default.
...
4. Add the canonical project skill sources and the idempotent setup/discovery checks.
...
- Should the baseline `project-verify` skill expose only safe read-only checks, or may it run the targeted validation commands automatically after confirmation?
