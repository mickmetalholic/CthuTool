...
checkout files or the global installation. For the running managed or local
source, the repository and ref default to the checkout's existing origin and symbolic
...
For `mode: local`, default update/check uses the linked Git checkout. A safe
update fast-forwards it to the planned commit without touching the managed
checkout or reinstalling global `chc`. Unrelated untracked files are preserved;
tracked edits, a divergent branch, or a path Git would overwrite block the
update. After editing CLI source locally, refresh the committed bundle with
`pnpm --filter @cthutool/cli dev`. Run either installer with
`CHC_INSTALL_MODE=remote` to switch back to managed mode.
...
The updater refuses unsafe dirty, diverged, or invalid-bundle targets and never
...
The CLI never prints or stores the launch ticket.
...
binaries, and preserves environment selection, browser profiles, and logs.
`--purge` removes those categories too and requires an interactive confirmation
