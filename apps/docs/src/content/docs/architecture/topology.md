...
Loopback bridge -----------------> Local Agent -> host Chrome + local profiles
                                      |
                                      +---- private-network WSS ----> Backend Agent Registry

Third-party App -> @cthutool/browser-client -> Backend Public Browser API
                                                   |
                                                   +-> bounded command -> Local Agent

External Web/operator HTTP -> Cloudflare Access/Tunnel -> private ingress -> Backend
...
host Chrome, and environment-scoped browser profiles. External Web/operator
HTTP access must use Cloudflare Access/Tunnel; the Agent WebSocket remains
private-network only. Direct public Backend port exposure is unsupported.
